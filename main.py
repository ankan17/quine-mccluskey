...
class QuineMccluskey():
    ...
    def _convert_to_binary(self, input_function):
        ...
        """Groups the minterms into binary form padded with 0's to the left according to the number of literals"""
    ...
    def _find_prime_implicants(self):
        ...
        implicant_table = [[{x: 0 for x in self.groups[i]} for i in range(self.literals+1)]]
        cur_round = 1
        ...
        while len(implicant_table[-1]) != 1:
            ...
            for i in range(len(cur_implicant_list) - 1):
                cur_implicant_group = {}
                for (x, y) in [(a, b) for a in cur_implicant_list[i].keys() for b in cur_implicant_list[i+1].keys()]:
                    bit_comparison = [1 if x[j] != y[j] else 0 for j in range(self.literals)]
                    if bit_comparison.count(1) == 1:
                        pos = bit_comparison.index(1)
                        cur_implicant_group.update({x[:pos] + '_' + x[pos+1:]: 0})
                        cur_implicant_list[i][x], cur_implicant_list[i+1][y] = 1, 1
    ...
    def _generate_pi_chart(self):
        ...
        pi_chart = [[pi, [0 for pi in self.input_function]] for pi in self.prime_implicants]
        ...
        for row in pi_chart:
            ...
            for j in range(self.literals):
                if pi[j] == '1':
                    values = list(map(lambda x: x+2**(self.literals-1-j), values))
                elif pi[j] == '_':
                    values += list(map(lambda x: x+2**(self.literals-1-j), values))
    ...
    def _find_essential_pi(self):
        ...
        """Finds the essential prime implicants and removes them from the prime implicant chart.
        Returns False if any essential prime implicants were found, True otherwise"""
            ...
            while start < end:
                ...
                if row[0] in essential_prime_implicants:
                    for j in range(num_of_minterms):
                        if row[1][j] == 1:
                            if j not in columns_to_remove: columns_to_remove.append(j)
    ...
    def _remove_dominating_columns(self):
        ...
        """Removes dominating columns and returns a boolean indicating whether column dominance was exhibited"""
        ...
        if self.prime_implicant_chart != []:
            ...
            for i in range(len(columns)):
                for col_to_be_compared in columns[:i] + columns[i+1:]:
                    if self._compare(columns[i], col_to_be_compared) == "Dominating":
                        if i not in columns_to_remove:
                            columns_to_remove.append(i)
            columns_to_remove.sort()
            if columns_to_remove != []: flag = False
            for row in self.prime_implicant_chart:
                for col in columns_to_remove:
                    row[1].pop(col - columns_to_remove.index(col))
            for col in columns_to_remove:
    ...
    def _remove_dominated_rows(self):
        ...
        """Removes dominated rows and returns a boolean indicating whether row dominance was exhibited"""
        ...
        if self.prime_implicant_chart != []:
            ...
            for i in range(len(rows)):
                for row_to_be_compared in rows[:i] + rows[i+1:]:
                    if self._compare(rows[i], row_to_be_compared) == "Dominated":
                        if i not in rows_to_remove:
                            rows_to_remove.append(i)
            if rows_to_remove != []: flag = False
            for row in rows_to_remove:
                self.prime_implicant_chart.pop(row - rows_to_remove.index(row))
                rows.pop(row - rows_to_remove.index(row))

            # Uncomment the following section if you wish to remove identical rows
    ...
    def _compare(self, list1, list2):
        ...
        """Helper method to compare two lists and tell if they are dominating, dominated or Incomparable"""
    ...
    def _solve_pi_chart(self):
        ...
        """Reduces the prime implicant chart by finding essential prime implicants, deleting dominating columns and
        deleting dominated rows in this order, iterated until the prime implicant chart is empty or can not be further
        reduced. If the prime implicant chart is still not empty after reduction, it is solved by Petrick's Method"""
                ...
                for exp in final_expression:
                    for elem in P[i]:
                        if elem not in exp:
                            ...
                            temp = exp
                        if temp not in new_exp: new_exp.append(temp)
                final_expression = new_exp

            for pair in combinations(final_expression, 2):
                if set(pair[0]).issubset(pair[1]):
                    if pair[1] in final_expression: final_expression.remove(pair[1])
                elif set(pair[1]).issubset(pair[0]):
                    if pair[0] in final_expression: final_expression.remove(pair[0])

            min_cover_size = min([len(e) for e in final_expression])
            solutions = []
            for exp in final_expression:
                if len(exp) == min_cover_size:
                    solutions.append(self.solutions[0] + [self.prime_implicant_chart[e-1][0] for e in exp])
...
if __name__ == "__main__":
    ...
    input_function = list(map(int, input("Enter the boolean SOP function: ").split()))
    input_function.sort()
